...
class DBManager:
    ...
    def get_companies_and_vacancies_count(self, params):
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT employers.name, COUNT(vacancies.employer_id) AS vacancies_count 
                FROM employers 
    ...
    def get_avg_salary(self, params):
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT ROUND(AVG(salary)) AS salary_avg FROM vacancies"""
                )
    ...
    def get_vacancies_with_higher_salary(self, params):
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT * FROM vacancies WHERE salary > (select AVG(salary) 
                            FROM vacancies)"""
                )
    ...
    def get_vacancies_with_keyword(self, keyword, params):
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"""SELECT * FROM vacancies WHERE name ILIKE '%{keyword}%' """
                )
